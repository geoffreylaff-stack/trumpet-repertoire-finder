...
// begins: a specific instrument name — a trumpet-family member, per
// firstFamilyMentionIndex, or any other real instrument (flute, oboe,
...
const OTHER_INSTRUMENT = String.raw`(?:piccolo|flutes?|oboes?|clarinets?|bassoons?|horns?|trumpets?|cornets?|trombones?|tubas?|timpani|harps?|violins?|violas?|cellos?|double\s+basses?|saxophones?|celesta|organ|pianos?)`;
...
// the instrument name itself ("2 cornets"'s "cornets"), not any count in
// front of it — cutting there would drop a stated, exact count and leave
// the parser to guess at a bare plural instead. Back up over it when present.
...
for (const w of imslp.works ?? []) {
  ...
  // to play: a chamber transcription is not the orchestral score, and letting
  // the two collide dropped the arrangement wherever a better source carried
  // the original. Only IMSLP supplies arrangements, so originals keep their keys.
 ...
 * recorded the death. The index's own data settles it. Of the composers with a
 * birth year and no death year, twenty-five were born before 1900 and the rest
 * in 1920 or later — with nobody at all in between. That gap is wide enough
 * that the boundary needs no judgement: born from 1900 onwards with no death
 * recorded means living, and earlier than that means a death nobody wrote down.
 *
 * Those earlier ones are shown as "1556–?" rather than "b. 1556", because
 * beside a list where "(b. 1969)" means a composer you could write to, a bare
