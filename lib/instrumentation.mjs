 ...
 *
 * A comma only starts a new clause when what follows carries a cue of its own
 * and the clause so far already had one. Otherwise the comma is punctuation
 * inside a single instruction: "2nd, 3rd and 4th doubling flugelhorn" lists
 * three chairs taking one instrument, not three separate doublings, and a
 * remark tacked on after a doubling belongs to the doubling before it.
 */
function doublingClauses(aside) {
  const clauses = [];
  for (const chunk of aside.split(/[,;]/)) {
    const last = clauses.at(-1);
    if (last !== undefined && !(DOUBLING_CUE.test(chunk) && DOUBLING_CUE.test(last))) {
...
function trailingRange(segment, afterIndex) {
  const m = /^\s*(\d+)\s*[-–—]\s*(\d+)\b/.exec(segment.slice(afterIndex));
  if (!m) return null;
  const [lo, hi] = [parseInt(m[1], 10), parseInt(m[2], 10)];
  if (!(hi > lo) || hi - lo > 11) return null; // not a part range
  ...
  return { count: hi - lo + 1, length: m[0].length };
}
...
export function parseInstrumentation(text) {
  ...
  for (const rawSegment of splitSegments(normalised)) {
    ...
    for (;;) {
      ...
      // Consume the name together with any part range written after it, so the
      // next instrument in a run-on list cannot inherit that range's last
      // number as its own count.
      ...
      scan = before + ' '.repeat(consumedEnd - m.index) + scan.slice(consumedEnd);
      lastEnd = consumedEnd;
    ...
    for (const aside of asides) {
      ...
      for (const { key, player } of readDoublings(stripTranspositions(aside))) {
