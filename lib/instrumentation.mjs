...
export function parseInstrumentation(text) {
  ...
  for (const rawSegment of splitSegments(normalised)) {
    ...
    //
    // "dbl." is the ordinary abbreviation in a score, and a part list often says
    // a player "includes" the second instrument rather than doubles on it. Both
    ...
    // treating it as one would claim a work needs a cornet when the trumpeter
    // may simply play it on the trumpet.
    for (const aside of asides) {
      if (!/doubl|\bdbl|includ|also|alternat|switch|=|raddoppi/i.test(aside)) continue;
